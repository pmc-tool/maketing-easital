        ...
        for ($i = 0; $i < $countwords; $i++) {
            if (3500 * $i + 4000 > strlen($page)) {
                try {
                    ...
                    if (strlen(substr($page, 3500 * $i)) > 10) {

                        $chatpdf = new PdfData;

                        $chatpdf->chat_id = $chat->id;
                        $chatpdf->content = substr($page, 3500 * $i);
        ...
        try {
            if ($type == 'chat') {
                ...
                foreach (explode("\n", $response->getBody()->getContents()) as $chunk) {
                    if (strlen($chunk) > 5 && $chunk != 'data: [DONE]' && isset(json_decode(substr($chunk, 6))->choices[0]->delta->content)) {
                        echo "event: data\n";
                        echo 'data: ' . json_encode(['message' => json_decode(substr($chunk, 6))->choices[0]->delta->content]) . "\n\n";
                        flush();
                    }
                }
            }
