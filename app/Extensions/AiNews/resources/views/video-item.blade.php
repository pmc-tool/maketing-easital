...
>
    @if ($entry['status'] === 'completed')
        @if (!empty($entry['title']))
            <p class="mb-2 truncate text-xs font-semibold text-heading-foreground" title="{{ $entry['title'] }}">
                {{ $entry['title'] }}
            </p>
        @endif
        <p class="mb-2.5 text-2xs font-medium text-heading-foreground">
            {{ isset($entry['duration']) ? substr($entry['duration'], 0, 7) : '0:00' }}
        </p>
        <img src="{{ $entry['gif_url'] ?? '' }}" alt="{{ $entry['title'] ?? '' }}">
        ...
        <div class="lqd-image-result-actions mt-auto flex w-full items-center justify-center gap-3">
            ...
            <x-button
                ...
                variant="ghost-shadow"
                size="none"
                :download="basename($entry['video_url'] ?? 'news-video.mp4')"
                :href="$entry['video_url'] ?? '#'"
                :disabled="!($entry['video_url'] ?? null)"
            >
            ...
            <x-button
                ...
                variant="ghost-shadow"
                size="none"
                onclick="return confirm('{{ __('Are you sure? This will permanently delete this news video.') }}')"
                :href="LaravelLocalization::localizeUrl(route('dashboard.user.ai-news.delete', $entry['video_id']))"
            >
