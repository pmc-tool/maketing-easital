@if (filled($list))
    ...
    >
        @foreach ($list as $entry)
            @include('ai-news::video-item', ['entry' => $entry, 'loop' => $loop])
        @endforeach
        ...
        >
            <div class="ai-avatar-videos-modal-backdrop absolute inset-0 bg-black/20 backdrop-blur-sm" @click="setVideoSrc(null)"></div>
            <button
            ...
            <div class="ai-avatar-videos-modal-video-wrap relative z-1 flex w-full items-center justify-between gap-4 p-5">
                <button
                    class="size-10 z-3 inline-grid place-content-center rounded-full bg-white/80 text-black transition-all hover:bg-red-500 hover:text-white disabled:pointer-events-none disabled:opacity-50"
                    @click="prevVideo()"
                    type="button"
                ...
                <div class="flex aspect-video max-h-[80vh] max-w-[calc(100vw-5rem)] grow items-center justify-center">
                    <video class="ai-avatar-videos-modal-video max-h-full max-w-full" src="#" x-bind:src="videoSrc" x-ref="video" controls></video>
                </div>
                <button
                    class="size-10 z-3 inline-grid place-content-center rounded-full bg-white/80 text-black transition-all hover:bg-red-500 hover:text-white disabled:pointer-events-none disabled:opacity-50"
                    @click="nextVideo()"
