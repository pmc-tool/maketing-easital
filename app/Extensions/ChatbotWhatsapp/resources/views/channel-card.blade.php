@php
	$image = 'vendor/chatbot-multi-channel/icons/whatsapp.svg';
	$image_dark_version = 'vendor/chatbot-multi-channel/icons/whatsapp-light.svg';
	$darkImageExists = file_exists(public_path($image_dark_version));
@endphp

<x-modal
	class:modal-head="border-b-0"
	class:modal-body="pt-3"
	class:modal-container="max-w-[600px]"
>
	<x-slot:trigger
		class="rounded-sm lqd-social-media-card flex flex-col  text-heading-foreground transition-all hover:scale-105 hover:border-heading-foreground/10 hover:shadow-lg hover:shadow-black/5"
		variant="outline"
		size="lg"
		type="button"
	>
		<figure class="mb-8 w-9 transition-all group-hover/card:scale-125">
			<img
				@class([
					'w-full h-auto',
					'dark:hidden' => $darkImageExists,
				])
				src="{{ asset($image) }}"
				alt="whatsapp"
			/>
			@if ($darkImageExists)
				<img
					class="hidden h-auto w-full dark:block"
					src="{{ asset($image_dark_version) }}"
					alt="whatsapp"
				/>
			@endif
		</figure>
		<h4 class="mb-2 text-lg text-inherit">
			Whatsapp
		</h4>

	</x-slot:trigger>

	<x-slot:modal>
		<h3 class="mb-3.5">
			Whatsapp
		</h3>
		<p class="mb-7 text-heading-foreground/60">
			@lang('Connect your WhatsApp Business account using the Meta WhatsApp Cloud API.')
		</p>

		<form id="storeForm-whatsapp" class="" action="{{ route('dashboard.chatbot-multi-channel.whatsapp.store') }}">
			<input hidden name="channel" value="whatsapp">
			<input hidden name="user_id" value="{{ \Illuminate\Support\Facades\Auth::id() }}">
			<div class="mb-3">
				<x-forms.input
					:label="__('Phone Number ID')"
					name="credentials[whatsapp_phone_number_id]"
					size="lg"
					required
					placeholder="e.g. 123456789012345"
				>
				</x-forms.input>
				<p class="mt-1 text-xs text-heading-foreground/40">
					@lang('Found in Meta WhatsApp Business > API Setup')
				</p>
			</div>
			<div class="mb-3">
				<x-forms.input
					:label="__('Access Token')"
					name="credentials[whatsapp_access_token]"
					size="lg"
					required
					type="password"
				>
				</x-forms.input>
				<p class="mt-1 text-xs text-heading-foreground/40">
					@lang('Permanent token from Meta Business Settings > System Users')
				</p>
			</div>
			<div class="mb-3">
				<x-forms.input
					:label="__('Verify Token')"
					name="credentials[whatsapp_verify_token]"
					size="lg"
					required
					placeholder="e.g. my_custom_verify_string"
				>
				</x-forms.input>
				<p class="mt-1 text-xs text-heading-foreground/40">
					@lang('A custom string you define — must match what you enter in Meta App Dashboard webhook settings')
				</p>
			</div>
			<div class="mb-3">
				<x-forms.input
					:label="__('App Secret')"
					name="credentials[whatsapp_app_secret]"
					size="lg"
					required
					type="password"
				>
				</x-forms.input>
				<p class="mt-1 text-xs text-heading-foreground/40">
					@lang('Found in Meta App Dashboard > Settings > Basic')
				</p>
			</div>
			<div class="mb-5 rounded-lg border p-3">
				<label class="mb-1 block text-sm font-medium text-heading-foreground">
					@lang('Webhook URL')
				</label>
				<p class="break-all text-sm text-heading-foreground/60">
					@lang('After saving, copy the webhook URL from the channel card and paste it into your Meta App Dashboard > WhatsApp > Configuration > Callback URL.')
				</p>
			</div>
			@if ($app_is_demo)
				<x-button
					type="button"
					onclick="return toastr.info('This feature is disabled in Demo version.');"
				>
					{{ __('Add Channel') }}
				</x-button>
			@else
				<x-button
					type="button"
					x-on:click="storeChannel('storeForm-whatsapp')"
					form="storeForm-whatsapp"
					size="lg"
				>
					<span x-show="storeChannelFetch">
						Loading...
					</span>
					<span x-show="!storeChannelFetch">
						{{ __('Add Channel') }}
					</span>
				</x-button>
			@endif
		</form>
	</x-slot:modal>
</x-modal>
