...
class MessengerConversationService
{
    ...
    public function storeConversation(): Builder|Model|ChatbotConversation
    {
        ...
        $senderName = data_get($this->payload, 'sender.id');

        $profile = null;

        try {
            $channel = ChatbotChannel::find($this->channelId);
            $accessToken = data_get($channel, 'credentials.access_token');

            if ($accessToken) {
                $bot = new \App\Extensions\ChatbotMessenger\System\Helpers\SimpleMessengerBot($accessToken);
                $profile = $bot->getUserProfile($this->getCustomerChannelId());

                if ($profile) {
                    $firstName = $profile['first_name'] ?? '';
                    $lastName = $profile['last_name'] ?? '';
                    $senderName = trim($firstName . ' ' . $lastName) ?: $senderName;
                }
            }
        } catch (Exception $e) {
            // Fall back to sender ID if profile fetch fails
        }
        ...
        // Create or update customer record with profile info
        if ($profile && ! $this->conversation->getAttribute('chatbot_customer_id')) {
            $customer = ChatbotCustomer::firstOrCreate([
                'chatbot_id'      => $this->chatbotId,
                'chatbot_channel' => 'messenger',
                'session_id'      => $this->getCustomerChannelId(),
            ], [
                'name'       => $senderName,
