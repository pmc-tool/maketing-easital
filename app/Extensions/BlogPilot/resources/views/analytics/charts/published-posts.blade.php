@php
	...
	if ($app_is_demo) {
		...
		foreach ($demo_agents as $agent) {
			$chart_data[] = [
				'label' => __(ucfirst($agent)),
				'id' => $agent,
				'chart_series' => [
					'name' => $agent,
	...
	if ($app_is_demo) {
		$agent = collect($chart_data)->firstWhere('id', 'Justy');

		$initialFilter = $facebook['chart_series']['name'] ?? 'Justy';
		$initialLabel = $facebook['label'] ?? __('Justy');
...
<div x-data='blogPilotPublishedPostsChart'>
	<x-card class:body="px-6">
		...
		>
			<h4 class="m-0 text-sm font-medium">
				@lang('Posts')
				<small class="opacity-55 text-4xs">@lang('includes all post statuses')</small>
			</h4>
		...
		<div
			class="min-h-56"
			id="blogpilot-published-posts-chart"
		></div>
	</x-card>
</div>
...
@push('script')
	<script>
		document.addEventListener('alpine:init', () => {
			Alpine.data('blogPilotPublishedPostsChart', () => ({
				chartData: @json($chart_data),
				activeFilter: "{{ $initialFilter }}",
				chart: null,
				chartEl: document.querySelector('#blogpilot-published-posts-chart'),
				counterEl: document.querySelector('#published-posts-counter .lqd-number-counter-value'),
