    ...
    (() => {
        document.addEventListener('alpine:init', () => {
            Alpine.data('blogPilotPosts', () => ({
                ...
                generationStatusEndpoint: '{{ route('dashboard.user.blogpilot.agent.api.generation-status') }}',
                ...
                pendingCounterEl: document.querySelector('#blogpilot-pending-posts-counter .lqd-number-counter-value'),
                scheduledCounterEl: document.querySelector('#blogpilot-scheduled-posts-counter .lqd-number-counter-value'),
                flickityData: null,
                cols: {
                    '(max-width: 767px)': 1,
                ...
                readyTextTemplate: "{{ ':generated of :total posts are being generated.' }}",
                ...
                get generationStatusLabel() {
                    ...
                    return '{{ __('Post generation done.') }}';
                },
                ...
                init() {
                    if ('Flickity' in window && this.$refs.postsCarousel) {
                        ...
                        this.flickityData = new Flickity(this.$refs.postsCarousel, {
                            cellSelector: '.blogpilot-post-item',
                    ...
                    window.addEventListener('blogpilot:post-created', this.externalRefreshHandler);
                    window.addEventListener('blogpilot:duplicate-post', event => {
                        if (!event.detail?.id) {
                            return;
                        }
                ...
                getSidedrawer() {
                    const editSidedrawerEl = document.querySelector('#blogpilot-sidedrawer');
                ...
                async fetchPost({
                    ...
                    url = url ?? `/dashboard/user/blogpilot/agent/api/posts?per_page=1&${query}`;
                ...
                async updatePost(postId) {
                    ...
                    const res = await fetch(`/dashboard/user/blogpilot/agent/api/posts/${postId}`, {
                        method: 'PUT',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Content-Type': 'application/json',
                    ...
                    const postEl = document.querySelector(`.blogpilot-post-item[data-post-id="${postId}"]`);
                    const postElAlpineData = postEl && Alpine.$data(postEl);
                    const dashboardCalendarEl = document.querySelector('#blogpilot-calendar');
                    ...
                    this.$dispatch('blogpilot-post-updated', {
                        post: data.post
                    });
                },
                ...
                async approvePost(postId) {
                    ...
                    const res = await fetch(`/dashboard/user/blogpilot/agent/posts/${postId}/approve`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Content-Type': 'application/json',
                    ...
                    this.$dispatch('blogpilot-post-approved', {
                        postId: postId
                    });
                },
                ...
                async rejectPost(postId) {
                    ...
                    const res = await fetch(`/dashboard/user/blogpilot/agent/posts/${postId}/reject`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Content-Type': 'application/json',
                    ...
                    this.$dispatch('blogpilot-post-rejected', {
                        postId: postId
                    });
                },
                ...
                async duplicatePost(postId) {
                    ...
                    try {
                        const res = await fetch(`/dashboard/user/blogpilot/agent/posts/${postId}/duplicate`, {
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Content-Type': 'application/json',
                ...
                async regeneratePostContent(postId) {
                    ...
                    try {
                        const res = await fetch(`/dashboard/user/blogpilot/agent/api/posts/${postId}/regenerate`, {
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Content-Type': 'application/json',
                ...
                onAjaxSuccess() {
                    ...
                    const newPosts = [...html.children || []].filter(el => el.classList.contains('blogpilot-post-item'));
                ...
                async filterPosts({
                    ...
                    let url = `/dashboard/user/blogpilot/agent/post-items?${params}${query ? `&${query}` : ''}`;
                    ...
                    if (this.$refs.postsCarousel && this.flickityData) {
                        this.flickityData.remove(
                            ...
                            .filter(element => !element.classList.contains('blogpilot-posts-carousel-load-more-wrap'))
                        );
