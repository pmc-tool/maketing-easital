...
<div
    id="blogpilot-posts-carousel"
    @class([
        '-mx-5 overflow-hidden [&_.flickity-viewport]:w-full [&_.flickity-slider]:w-full' => filled(
            $posts),
        'flex relative [&.flickity-resize_.blogpilot-post-item]:min-h-full',
    ])
    :class="{ 'animate-pulse': currentTasks.has('fetchingPosts') }"
    x-ref="postsCarousel"
>
    @include('blogpilot::components.posts.carousel.post-items', ['items' => $posts])

    @if ($posts->hasMorePages())
        <div
            class="blogpilot-posts-carousel-load-more-wrap blogpilot-post-item flex min-h-full w-full flex-shrink-0 flex-grow-0 basis-auto flex-col px-5 py-2 md:w-1/2 lg:w-1/4">
            <div class="flex min-h-full grow items-center justify-center p-3.5">
                <a
                    class="blogpilot-posts-carousel-load-more group inline-flex w-full flex-col items-center gap-2 p-10 text-center text-sm font-medium"
                    href="{{ route('dashboard.user.blogpilot.agent.post-items', ['page' => 2, 'post_style' => 'carousel']) }}"
