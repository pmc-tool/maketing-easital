@php
	...
	if ($app_is_demo) {
		...
		// Generate "All" data
		$all_data = array_map(fn() => rand(500, 2000), range(0, 11));
		$chart_data[] = [
			'label' => __('All'),
			'id' => '*',
			'chart_series' => [
		...
		foreach ($demo_platforms as $platform) {
			$chart_data[] = [
				...
				'chart_series' => [
					'name'   => $platform,
					'data'   => array_map(fn() => rand(100, 500), range(0, 11)),
					'color'  => $brand_colors[$platform] ?? 'hsl(var(--primary))',
				],
				'today_growth' => rand(10, 100),
			];
		}
	}
...
<div x-data='socialMediaAgentAudienceGrowthChart'>
	<x-card class:body="px-6">
		...
		<div
			class="min-h-56"
			id="social-media-agent-audience-growth-chart"
		></div>
	</x-card>
</div>
...
@push('script')
	<script>
		document.addEventListener('alpine:init', () => {
			Alpine.data('socialMediaAgentAudienceGrowthChart', () => ({
				chartData: @json($chart_data),
				activeFilter: "{{ $initialFilter }}",
				chart: null,
				chartEl: document.querySelector('#social-media-agent-audience-growth-chart'),
				counterEl: document.querySelector('#audience-growth-counter .lqd-number-counter-value'),
				...
				init() {
					this.chart = new ApexCharts(this.chartEl, {
						series: this.chartData.filter(data => data.chart_series.name !== 'all').map(data => data.chart_series),
				...
				setActiveFilter(filter) {
					...
					this.chartData.forEach(data => {
						...
						if (this.activeFilter === 'all') {
							return this.chart.showSeries(name);
						}
					...
					if (counterElData && counterElData.updateValue) {
						counterElData.updateValue({
							value: this.chartData.find(data => data.chart_series.name === this.activeFilter)?.today_growth ?? 0
						})
					}
				}
			}))
