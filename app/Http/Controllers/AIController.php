...
class AIController extends Controller
{
    ...
    public function imageOutput($param, $post, $user): JsonResponse
    {
        ...
        $engineCheck = match ($param['image_generator']) {
            ...
            'novita'                       => EngineEnum::NOVITA->value,
        ...
        try {
            ...
            if ($param['image_generator'] === 'novita') {
                $model = EntityEnum::NOVITA;
            }
    ...
    private function getDefaultModel(?EngineEnum $engine): ?EntityEnum
    {
        return match ($engine) {
            ...
            EngineEnum::NOVITA           => EntityEnum::NOVITA,
    ...
    private function processImageGeneration(?EngineEnum $engine, ?EntityEnum $model, array $param): array
    {
        return match (true) {
            ...
            $engine === EngineEnum::NOVITA                                                                                 => $this->processNovitaImage($model, $param),
            default                                                                                                         => throw new Exception(__('Invalid AI Engine')),
        };
    }
    ...
    /**
     * Process image generation using Novita AI API.
     *
     * @throws Exception
     */
    private function processNovitaImage(?EntityEnum $model, array $param): array
    {
        $apiKey = setting('novita_api_key');

        if (empty($apiKey)) {
            throw new Exception(__('Novita API key is not configured.'));
        }

        $prompt = $param['description'] ?? ($param['stable_description'] ?? '');

        if (empty($prompt)) {
            throw new Exception(__('Prompt is required for image generation.'));
        }

        // Novita API enforces a 1024-character limit on prompts
        ...
        if (! $response->successful()) {
            throw new Exception(__('Novita API request failed: ') . $response->body());
        }

        $taskId = $response->json('task_id');

        if (empty($taskId)) {
            throw new Exception(__('Novita API did not return a task ID.'));
        }
        ...
        for ($attempt = 0; $attempt < $maxAttempts; $attempt++) {
            ...
            $statusResponse = Http::withHeaders([
                'Authorization' => 'Bearer ' . $apiKey,
                'Content-Type'  => 'application/json',
            ])->get('https://api.novita.ai/v3/async/task-result', [
                'task_id' => $taskId,
            ]);
            ...
            $taskStatus = $statusResponse->json('task.status');

            if ($taskStatus === 'TASK_STATUS_SUCCEED') {
                $images = $statusResponse->json('images');

                if (empty($images) || empty($images[0]['image_url'])) {
                    throw new Exception(__('Novita returned no images.'));
                }
                ...
                if ($imageContent === false) {
                    throw new Exception(__('Failed to download Novita image.'));
                }

                $nameOfImage = 'novita-' . uniqid() . '.png';
            ...
            if ($taskStatus === 'TASK_STATUS_FAILED') {
                $reason = $statusResponse->json('task.reason', 'Unknown error');
                throw new Exception(__('Novita image generation failed: ') . $reason);
            }
        }

        throw new Exception(__('Novita image generation timed out after ') . ($maxAttempts * $delay) . __(' seconds.'));
    }
